...
def send_resource_freed_msg_to_next_in_line(user: User, skip_rety:bool=False) -> None:
    ...
    while retry_cnt < global_vars.reconnexion_retries and not skip_rety:
        try:
            time.sleep(global_vars.resource_free_delay/global_vars.reconnexion_retries)
            if user.is_trying_to_reconnect():
                user_retry:User = User.get_user_reconnexion_attempt(user)
                elapsed_time = int(datetime.utcnow().timestamp()) - int(user.get_user_info("start_time"))
                timeout = max(int(user.get_user_info("timeout")) - elapsed_time, 0)
                user_retry.update({"timeout":timeout, "start_time":user.get_user_info("start_time")})
                ...
                msg = ComProtocole.generate_msg(ComHeaders.UPDATE, user_retry.json_dump())
                client.sendall(msg.encode())
                send_resource_freed_msg(
                    user_retry,
                    msg="Resource re attributed to you, remaining time: {}s".format(timeout)
                )
                log_str = "{} trying to reconnect, granting resource".format(user_retry.get_user_name())
                Log.log(Log.dbg_level, log_str)
                break
            retry_cnt += 1
    ...
    if retry_cnt >= global_vars.reconnexion_retries or skip_rety:
        if len(User.get_user_list()) > 1 and user == User.get_first_user_in_line():
            ...
            Log.log(Log.dbg_level, "giving resource to {}".format(next_user.get_user_name()))
...
def add_client_to_user_list(user: User, payload: str) -> None:
    ...
    user.update({"start_time":datetime.utcnow().replace(microsecond=0).timestamp()})
